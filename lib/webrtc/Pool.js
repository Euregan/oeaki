...
class Pool {
    constructor(websocketSignalingUrl, lobbyId) {
        this.uuid = uuid();
        ...
        this.server.listen(({ type, id }) => {
            switch (type) {
                case 'hello':
                    if (!this.peers[id]) {
                        this.peers[id] = new Peer(
                            this.server.filter(this.uuid, id),
                ...
                case 'welcome':
                    ...
                    break;
                case 'lobby-subscription-reply': {
                    this.server.send({ type: 'hello', id: this.uuid });
                }
            }
        });

        this.server.send({ type: 'lobby-subscription', id: lobbyId });
    }
