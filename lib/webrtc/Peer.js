class Peer {
    constructor(signaling, channels, onShutdown, offerer) {
        ...
        channels.forEach((channel) => {
            ...
            this.channels[channel].onopen = () => {
                const queuedMessages = this.queues[channel];
                this.queues[channel] = [];
                queuedMessages.forEach((message) => this.send(channel, message));
            };
        });
    }
    ...
    send(channel, message) {
        if (this.channels[channel].readyState === 'connecting') {
            this.queues[channel].push(message);
        }
